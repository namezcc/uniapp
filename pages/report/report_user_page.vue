...
<script>
import { ReportUserName, ReportUserType } from '../../common/define_const'
...
import util_common from '../../common/util_common'
	...
	export default {
		data() {
			return {
				reportType:ReportUserType.fanzui,
				reportName:ReportUserName[0],
				txtContent:"",
				imageValue:[],
				imageStyle: {
					borderRadius: '10px',
		...
		methods: {
			toSelectPage() {
				uni.navigateTo({
					url:"/pages/report/select_page?type=1",
					events:{
						selectData:(res)=>{
							// console.log("select data ",res)
							this.reportType = res
							this.reportName = ReportUserName[res]
						}
					}
				})
			},
